...
export function SurveyAttachmentsCard({ activity, setActivity }) {
  ...
  // New: local selection for not-yet-uploaded files
  const [selectedFiles, setSelectedFiles] = useState([]); // [{id,file,previewUrl}]

  const existingSurveyAttachments = useMemo(
  ...
  const handleAttachmentsUpload = async (e) => {
    ...
    formData.append('attachmentTarget', 'survey');
    ...
    try {
      ...
      const updatedSurvey = updatedActivity?.survey || activity.survey || {};
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Upload survey photos and view existing attachments.
            </p>
          </div>
          {existingSurveyAttachments.length > 0 && (
            <Badge variant="outline" className="text-xs">
              {existingSurveyAttachments.length} file
              {existingSurveyAttachments.length > 1 ? 's' : ''}
            </Badge>
          )}
        </CardHeader>
        ...
        <CardContent className="space-y-4 text-sm">
          ...
          <form className="space-y-3" onSubmit={handleAttachmentsUpload}>
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-muted-foreground">
                Images will be stored as survey-level attachments.
              </p>
            </div>
          </form>

          {/* New: Selected (NOT YET UPLOADED) previews */}
          ...
          {existingSurveyAttachments.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {existingSurveyAttachments.map((url, idx) => {
